 ...
 * This drawing algorithm is based on my friend Hiroki (http://youdrawingyou.com/sketches/hiroki)
 */

import processing.pdf.*;

String imgSrc = "img/hiroki.jpg";
String outputFile = "output/hiroki.png";
String outputPDF = "output/hiroki.pdf";
...
void setup() {
  ...
  // create a group of Hirokis 
  theHirokiGroup = new HirokiGroup();
...
class HirokiGroup
{
  ...
  ArrayList<Hiroki> group;

  HirokiGroup () {          
    group = new ArrayList<Hiroki>();
    for(int i=0; i<groupSize; i++) {
      group.add(new Hiroki());
    }
  }

  void research() {   
    for (int i = group.size()-1; i >= 0; i--) {    
      Hiroki hiroki = group.get(i);
      hiroki.research();
    }   
  } 
...
class Hiroki
{
  ...
  void research(){    
    ...
    float[] distances = {y-worldBorder, worldWidth-worldBorder-x, worldHeight-worldBorder-y, x-worldBorder};
    float radius = min(distances);
    x += radius;
    y += radius;
    float[] c = getCenter(x, y, radius, angle),
            p = Math.rotatePoint(x, y, c[0], c[1], 90);
    Space space = new Space(p[0], p[1]);
    boolean first = true;
          
    while(space.isWithinWorld() && space.isEmpty() && radius>minRadius) {
      if (first) {
        first = false;
      } else {
        drawArc(c[0], c[1], radius*2, radius*2, angle, angle+90);
        space.researchAround();
      }      
      angle = Math.normalizeAngle(angle + 90);
      radius = radius / Math.phi();
      c = getCenter(space.getX(), space.getY(), radius, angle);
      p = Math.rotatePoint(space.getX(), space.getY(), c[0], c[1], 90);
      space = new Space(p[0], p[1]);
    }
  }
  
  float[] getCenter(float x, float y, float r, float angle){
