 ...
 * This drawing algorithm is based on my friend Angela (http://youdrawingyou.com/sketches/angela)
 */

import processing.pdf.*;

String imgSrc = "img/angela.jpg";
String outputFile = "output/angela.png";
String outputPDF = "output/angela.pdf";
boolean savePDF = false;

int worldWidth = 675;
int worldHeight = 900;
float worldBorder = 50;
int spaceIterator = 0;
...
void setup() {
  ...
  // create a group of Angelas 
  theAngelaGroup = new AngelaGroup();
...
class AngelaGroup
{
  ...
  ArrayList<Angela> group;

  AngelaGroup () {
    float centerX = worldWidth/2,
          centerY = worldHeight/2;    
    group = new ArrayList<Angela>();
    for(int i=0; i<groupSize; i++) {
      group.add(new Angela(centerX, centerY));
    }
  }

  void grab() {   
    for (int i = group.size()-1; i >= 0; i--) {    
      Angela angela = group.get(i);
      angela.grab();
    }   
  } 
...
{
  int baseX = 2, baseY = 3;
  float minDistance = 20, maxDistance = 120, angleStep = 30;
  float centerX, centerY;
  
  Angela (float x, float y) {
    centerX = x;
    centerY = y;
  }
  ...
  void drawRandomBezier(float x1, float y1, float x4, float y4) {
    float distance = dist(x1, y1, x4, y4),
          a2 = random(1,360),
          a3 = random(1,360);
    float[] p2 = Math.translatePoint(x1, y1, a2, distance/2),
            p3 = Math.translatePoint(x4, y4, a3, distance/2);
  ...
  void grab(){
    ...
    if (space.isWithinWorld() && space.isEmpty()) {
      grabSpace(space, angle, maxDistance);
    }
  }
  
  void grabSpace(Space space, float angle, float distance) {
    if (distance < minDistance) return;
    
    float[] p = Math.translatePoint(space.getX(), space.getY(), angle, distance);
    Space s = new Space(p[0], p[1]);
    
    if (s.isWithinWorld() && s.isEmpty()) {
      drawRandomBezier(space.getX(), space.getY(), s.getX(), s.getY());
      space.grabAround();
      grabSpace(s, angle+angleStep, distance/random(1, 2));
      grabSpace(s, angle-angleStep, distance/random(1, 2));
    }
  }
